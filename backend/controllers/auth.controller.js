  ...
  try {
    ...
    if (!usernameRegex.test(username)) {
      return res
        ...
        .json({
          error_message:
            "Username must be 3-15 characters, alphanumeric or underscores only.",
        });
    }
    ...
    return res
      .status(500)
      .json({
        error_message: `Internal server error in signupController: ${
          e.message || e
        }`,
      });
  }
...
export const loginController = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({
        error_message: `Internal server error in loginController: ${
          e.message || e
        }`,
      });
  }
...
export const getMeController = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({
        error_message: `Internal server error in logoutController: ${
          e.message || e
        }`,
      });
  }
